...
<div class="container-fluid max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl font-bold text-gray-800">Chỉnh Sửa Banner Danh Mục</h1>
        </div>
        <p class="text-gray-600 ml-9">Cập nhật thông tin banner cho trang chủ</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-lg shadow-md p-8">
        <form action="{{ route('admin.category-banners.update', $categoryBanner) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="position"
                    ...
                    <option value="1" {{ old('position', $categoryBanner->position) == 1 ? 'selected' : '' }}>Vị trí 1 (Trên - Trái)</option>
                    <option value="2" {{ old('position', $categoryBanner->position) == 2 ? 'selected' : '' }}>Vị trí 2 (Dưới - Trái)</option>
                    <option value="3" {{ old('position', $categoryBanner->position) == 3 ? 'selected' : '' }}>Vị trí 3 (Trên - Phải)</option>
                    <option value="4" {{ old('position', $categoryBanner->position) == 4 ? 'selected' : '' }}>Vị trí 4 (Dưới - Phải)</option>
                </select>
                @error('position')
            ...
            <div class="mb-6">
                ...
                <div class="border border-gray-300 rounded-lg p-4 bg-gray-50">
                    <img src="{{ Storage::url($categoryBanner->image) }}"
                        alt="{{ $categoryBanner->description }}"
            ...
            <div class="mb-6">
                <label class="block text-gray-700 font-semibold mb-2">
                    Thay Đổi Ảnh Banner <span class="text-gray-500 text-sm">(Tùy chọn)</span>
                </label>
            ...
            <div class="mb-6">
                ...
                <input type="text"
                    ...
                    value="{{ old('description', $categoryBanner->description) }}">
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox"
                        ...
                        {{ old('is_active', $categoryBanner->is_active) ? 'checked' : '' }}>
                    <span class="ml-2 text-gray-700 font-semibold">Hiển thị banner</span>
                </label>
            </div>
